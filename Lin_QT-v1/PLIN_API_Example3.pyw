...
#  Language: Python 3.3
#  Last Change: 23.04.2014 / Keneth Wagner
#  ------------------------------------------------------------------
#
...
# *****************************************************************
    ...
    def insertMsgEntry(self, newMsg):
        ...
        with self._lock:
            ...
            #m_LastMsgsList 是个列表，把新信息对象加入列表
    ...
    def formatMsgStatus(self, msgStatus):
        ...
        # Length
        strTemp = str(msgStatus.m_Msg.Length) #无符号的数字转换为字符
        separator = " " * (self.m_ListColSpace[self.COL_LEN] - len(strTemp))
        if (len(separator) == 0):
            separator = seperatorMinimum
        result = result + (strTemp + separator)
        # Data
        strTemp = msgStatus.getDataString() #得到数据域的值
